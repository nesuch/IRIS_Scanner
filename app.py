...
def build_chips_html(kw_tuples, original_query):
    ...
    for raw, clean in kw_tuples:
        ...
        payload = f"{raw}|{clean}"
        html += f"""<form method="POST" style="margin:0;" onsubmit="return handleDeepScanSubmit(this)"><input type="hidden" name="query" value="__DEEP_SCAN__:{payload}">
    ...
    if (len(clean_original.split()) > 1 or has_special_compound) and phrase_label not in shown_labels:
        html += f"""<form method="POST" style="margin:0;" onsubmit="return handleDeepScanSubmit(this)"><input type="hidden" name="query" value="__DEEP_SCAN__:{clean_original}|{clean_original}">
    ...
    if len(kw_tuples) > 1:
        ...
        html += f"""<form method="POST" style="margin:0;" onsubmit="return handleDeepScanSubmit(this)"><input type="hidden" name="query" value="__DEEP_SCAN__:{all_payload}">
